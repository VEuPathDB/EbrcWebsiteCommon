...
public class DetailTableLoader extends BaseCLI {
  ...
  protected void declareOptions() {
    ...
    addSingleValueOption(ARG_DETAIL_TABLE, true, null,
        "The name of the " + "detail table where the cached results are stored.");

    addSingleValueOption(ARG_TABLE_FIELD, false, null,
  ...
  public void execute() throws Exception {
    ...
    String detailTable = (String) getOptionValue(ARG_DETAIL_TABLE);
  ...
  private void dumpTable(DatabaseInstance appDb, String detailTable, TableField table, String idSql, RecordClass recordClass)
    ...
    DataSource updateDataSource = appDb.getDataSource();
    ...
    try {
      ...
      deleteRows(detailTable, idSql, pkNames, table.getName(), updateConnection);
      ...
      int[] counts = aggregateLocally(appDb, table, idSql, insertStmt, insertSql, pkColumns);
      long startCommit = System.currentTimeMillis();
      updateConnection.commit();
      long end = System.currentTimeMillis();

      logger.info(
          "Dump table [" + table.getName() + "] done.  Created and inserted " + counts[0] + " (" + counts[1] +
              " detail) rows in " + ((end - start) / 1000.0) + " seconds.  Cumulative insert time was " +
              (counts[2] / 1000) + " seconds.  Commit was " + (end - startCommit) / 1000);
    }
  ...
  private void deleteRows(String detailTable, String idSql, String pkNames, String fieldName,
    ...
    logger.debug("\n" + sql);
    SqlUtils.executeUpdate(connection, sql.toString(), fieldName + "__api-report-detail-delete");
    long end = System.currentTimeMillis();
    logger.info("Deleted rows for [" + fieldName + "] in " + ((end - start) / 1000.0) + " seconds");
  }

  private int[] aggregateLocally(DatabaseInstance appDb, TableField table, String idSql,
  ...
  private String[] formatAttributeValues(ResultSet resultSet, TableField table)
    ...
    String[] formattedValuesArray = new String[fields.size()];
  ...
  private String formatValue(Map<String, String> formattedValuesMap, TableField table,
      AttributeField attribute, ResultSet resultSet)
      throws WdkModelException, SQLException, WdkUserException {
   ...
   * @param idSql
   */
  private long insertDetailRow(PreparedStatement insertStmt, DBPlatform platform, String insertSql,
      StringBuilder contentBuf, int rowCount, TableField table, String pk0, String pk1, String title,
      int pkCount) throws SQLException {
