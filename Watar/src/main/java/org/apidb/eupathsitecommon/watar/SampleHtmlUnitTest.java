...
public class SampleHtmlUnitTest {
  ...
  private static boolean searchTest() {
    ...
    try {
      currentPage = browser.getPage("http://www.google.com");
    }
    catch (Exception e) {
    ...
    try {
      ((HtmlTextInput) currentPage.getElementByName("q")).setValueAttribute(
        "qa automation");
      currentPage = (
        (HtmlElement) currentPage.getElementByName("btnG")
      ).click();
